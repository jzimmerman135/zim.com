...
float sdPlane(vec3 p)
{
    ...
    d = p.y + sin(p.z + iTime) * .2 + cos(p.x - iTime * 2.) * .4;
    return d;
}

float BoxDist(vec3 p)
{
    vec3 co = p - vec3(20, 3, 4);
    // co.yx *= Rot(iTime);
    return sdBox(co, vec3(.5, .5, 5));
}

vec2 GetDist(vec3 p)
{
    float dBox = BoxDist(p);
    // float dBox = sdBoard(co, vec3(0, .5, 5));
...
vec2 RayMarch(vec3 ro, vec3 rd)
{
    ...
    return vec2(dO, WATER);
}

// vec2 RflMarch(vec3 ro, vec3 rd)
// {
//     float dO=0.;
//     vec2 collision = vec2(0);
